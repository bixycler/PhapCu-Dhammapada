...
window.onresize = function(){
  let delta = getWidth() - windowWidth;
  console.debug('  Window resized ('+delta+'): '+getWidth()+' x '+window.innerHeight);
  if(Math.abs(delta) < 1){ return;}else{ windowWidth = getWidth(); }
...
new ResizeObserver(function(entries){
  ...
  if(Math.abs(dx) > 1){ contentWidth += dx; Body.style.width = contentWidth + 'px'; } 
...
function updateColumns(width){
  ...
  if(colnum < 2){
    ...
    if(isTouchDevice){ DualView.innerHTML = '';
...
async function setView(view){ MenuList.classList.add('hidden');
  ...
  if(view != 'ColumnsView') { updateGridTemplate(view); }
